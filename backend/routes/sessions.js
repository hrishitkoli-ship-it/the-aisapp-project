...
humanRouter.get('/', (req, res) => {
  const sessions = store.getSessions(req.params.projectId);
  res.json(sessions);
});
...
aiRouter.patch('/:sessionId', async (req, res) => {
...
aiRouter.post('/:sessionId/requests', async (req, res) => {
  const { projectId, sessionId } = req.params;
...
aiRouter.patch('/:sessionId/requests/:requestId', async (req, res) => {
